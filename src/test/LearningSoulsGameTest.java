...
public class LearningSoulsGameTest {
    ...
    public void testMain() {
        try {
            ...
            Assert.assertEquals("Gregooninator attacks Monster_1 with Basic Sword (ATTACK:9 | DMG : 7)", list[4]);
            Assert.assertEquals("[ Hero ]             Gregooninator        LIFE:  100      STAMINA:   30      PROTECTION:0.0       (ALIVE)", list[5]);
            Assert.assertEquals("[ Monster ]          Monster_1            LIFE:    3      STAMINA:   10      PROTECTION:20.0      (ALIVE)", list[6]);
            Assert.assertEquals("", list[7]);
            Assert.assertEquals("Hit enter key for next move > ", list[8]);
            Assert.assertEquals("Monster_1 attacks Gregooninator with Bloody Claw (ATTACK:146 | DMG : 100)", list[9]);
            Assert.assertEquals("[ Hero ]             Gregooninator        LIFE:    0      STAMINA:   30      PROTECTION:0.0       (DEAD)", list[10]);
            Assert.assertEquals("[ Monster ]          Monster_1            LIFE:    3      STAMINA:    5      PROTECTION:20.0      (ALIVE)", list[11]);
